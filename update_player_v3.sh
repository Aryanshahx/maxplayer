#!/usr/bin/env bash
# =============================================================================
# Max Player update v3: player bug fix + advanced features
# Run from your flutter project root, e.g.:  cd ~/IdeaProjects/maxplayer
#
# FIX: audio no longer keeps playing after leaving the player (also
#      pauses when the app goes to background).
# NEW: auto-hiding controls (4s), double-tap edges = seek +-10s,
#      right-side swipe = volume, aspect-ratio cycle button, +-10s
#      buttons, and resume-from-where-you-left-off (saved per video).
...
echo "  git add -A && git commit -m \"Player: fix audio-on-exit, add gestures, auto-hide, fit modes, resume playback\" && git push
