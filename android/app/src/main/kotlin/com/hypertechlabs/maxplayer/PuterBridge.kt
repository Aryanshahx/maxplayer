    ...
    fun signInSync(timeoutMs: Long): Boolean {
        ...
        main.post {
            ...
            wv.evaluateJavascript(
                "(function(){var o=document.createElement('div');o.id='mxSO';" +
                ...
                "justify-content:center;z-index:9999;color:#fff;font-family:sans-serif;';" +
                ...
                "try{if(puter.auth.isSignedIn()){var u=null;try{u=await puter.auth.getUser();}" +
                "catch(e){}MXP.onSignIn(true,(u&&(u.username||u.email))?" +
                "String(u.username||u.email):'guest');return;}" +
                "await puter.auth.signIn({attempt_temp_user_creation:true});" +
                "var u=null;try{u=await puter.auth.getUser();}catch(e){}" +
                "MXP.onSignIn(true,(u&&(u.username||u.email))?String(u.username||u.email):'guest');" +
                "}catch(e){var c=(e&&(e.error||e.code))?String(e.error||e.code):" +
                "((e&&e.message)?String(e.message):'cancelled');" +
                "MXP.onSignIn(false,c.slice(0,120));}});" +
                "document.getElementById('mxCB').addEventListener('click',function(e){" +
                "e.preventDefault();MXP.onSignIn(false,'cancelled');});})()", null)
        }
        return try {
