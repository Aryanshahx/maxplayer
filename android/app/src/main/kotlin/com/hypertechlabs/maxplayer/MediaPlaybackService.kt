...
class MediaPlaybackService : Service() {
    ...
    companion object {
        ...
        /** v32: live-service flag for the About → Diagnostics sheet. */
        @Volatile
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            val request = AudioFocusRequest.Builder(AudioManager.AUDIOFOCUS_GAIN)
                ...
                .setOnAudioFocusChangeListener { focusChange ->
                    ...
                    // v117: the transient-focus pause branch was removed -
                    // developer's call: background audio must keep playing
                    // by default; only a permanent focus LOSS pauses, GAIN
                    // resumes.
                }
                .build()
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        val thumbBmp: Bitmap? = if (!thumbPath.isNullOrEmpty() && File(thumbPath).exists()) {
            try {
                // v116: decode with downsampling - a 4K thumbnail decoded
                // full-size is a ~33 MB bitmap (OOM risk + Play's
                // BitmapFactory warning). Cap at ~960 px; the
                // notification only ever shows ~256.
                val bounds = BitmapFactory.Options().apply {
                    inJustDecodeBounds = true
                }
                BitmapFactory.decodeFile(thumbPath, bounds)
                var sample = 1
                while (bounds.outWidth / sample > 960) sample *= 2
    ...
    /** v113: swipe-closing the app from recents used to leave a zombie
     *  foreground service: ghost notification, held wake lock and no way
     *  to dismiss playback controls.
     *  v114 follow-up: while audio is actually PLAYING in the background
     *  the service must survive the swipe (MX Player / Spotify behavior -
     *  controls stay in the media notification), or background audio dies
     *  the moment the user closes the app. Tear down only when nothing
     *  is playing (the STOP action path stops earlier by itself). */
    ...
    private fun buildNotification(
        ...
        val builder = NotificationCompat.Builder(applicationContext, Notifications.CHANNEL_PLAYBACK_V2)
            ...
            .setShowWhen(false)
            // v113: swiping the paused notification away now also stops
            // the foreground service (was: ghost service + held wake lock).
