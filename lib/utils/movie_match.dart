...
/// Tells whether a TMDB movie exists in the user's OWN local video library
/// ("In my library" button on the Discover detail sheet — the moment
/// discovery ends in OUR player, offline).
///
...
/// differently-written names of the same movie normalize identically. Pure.
...
/// Returns null when there is no match — the UI then hides the
/// "In my library" button.
AssetEntity? findLocalMovie(
  String tmdbTitle,
  int? year,
  ...
  AssetEntity? titleOnly;
  for (final v in videos) {
    final title = v.title ?? '';
    if (normalizeMovieTitle(title) != want) continue;
    if (year != null && title.contains('$year')) return v;
    titleOnly ??= v;
  }
  return titleOnly;
}
