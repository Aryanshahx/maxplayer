...
/// Watch-time statistics — old-player look: headline week total, five stat
/// cards, a 7-day bar chart and the "Most watched" list. Data comes from
/// [WatchStatsStore], which the player updates every 5 seconds of playback.
class StatisticsScreen extends StatelessWidget {
  const StatisticsScreen({super.key});

  Future<_StatsBundle> _load() async {
    final days = await WatchStatsStore.instance.weekStats();
    final month = await WatchStatsStore.instance.lastNDays(30);
    final streak = await WatchStatsStore.instance.streakDays();
    ...
    return Scaffold(
      ...
      body: FutureBuilder<_StatsBundle>(
        ...
        builder: (context, snapshot) {
          ...
          final topWatchedFuture = WatchStatsStore.instance.topWatched();

          return FutureBuilder<List<WatchedVideo>>(
            future: topWatchedFuture,
            builder: (context, topSnap) {
              final topWatched = topSnap.data ?? const <WatchedVideo>[];
              final topMax = topWatched.isEmpty ? 1 : topWatched.first.seconds;
              return ListView(
                ...
                children: [
                  ...
                  if (topWatched.isNotEmpty) ...[
                    ...
                    for (var i = 0; i < topWatched.length; i++)
                      _TopWatchedRow(
                        rank: i + 1,
                        title: topWatched[i].title,
                        seconds: topWatched[i].seconds,
                        fraction: topWatched[i].seconds / topMax,
                      ),
                  ],
