...
/// id -> user-renamed title. Populated the moment a rename succeeds so the
/// grid shows the new name instantly, without waiting for the MediaStore
/// asset list to be re-queried (the tiles would otherwise keep the stale
/// AssetEntity.title until a full rescan).
final Map<String, String> kRenamedVideoTitles = {};

/// Display title for [asset]: the latest rename when one is known, else the
/// MediaStore title.
String renamedTitleFor(AssetEntity asset) =>
    kRenamedVideoTitles[asset.id] ?? asset.title ?? 'Video';
...
class _VideoGridState extends State<VideoGrid> {
  ...
  Future<void> _rename(AssetEntity a) async {
    try {
      ...
      // Show the new name instantly: the asset list's AssetEntity.title is
      // only re-read on a full rescan, so remember it and repaint this tile
      // now (the user otherwise has to pull-to-rescan to see the rename).
