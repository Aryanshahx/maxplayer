...
/// The "AI Suggestor" sheet: the user types their taste in plain words —
/// or taps a mood chip — the AI names real films, and each pick appears as
/// a tappable TMDB poster. Popping a pick hands it back to Discover, which
/// opens the detail sheet (local library match included).
...
class _AiSuggestSheetState extends State<AiSuggestSheet> {
  ...
  Future<void> _restoreSaved() async {
    ...
    final p = prefs.getString(_kSavedPromptKey);
    final jsonStr = prefs.getString(_kSavedPicksKey);
  ...
  Future<void> _suggest([String? preset]) async {
    ...
    setState(() {
      ...
      if (picks == null) {
        ...
        prefs.then((p) {
          p.setString(_kSavedPromptKey, q);
          p.setString(
              _kSavedPicksKey,
              jsonEncode(_picks
                  .map((m) => {
