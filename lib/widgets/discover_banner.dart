...
/// The library's "Discover" banner — a wide banner whose BACKGROUND is a
/// strip of the latest trending movie posters (dark scrim on top so the
/// text always reads).
///
/// The poster background MOVES — a slow marquee that slides one poster
/// every couple of seconds in a seamless loop (the set is drawn twice, so
/// wrapping back one full set is invisible). The strip is also draggable by
/// hand; auto-scrolling pauses while the user is holding it.
///
...
class _DiscoverBannerState extends State<DiscoverBanner> {
  ...
  Future<void> _boot() async {
    ...
    if (!mounted || AppConfig.tmdbToken.isEmpty) return;
    List<TmdbMovie> list;
    try {
    ...
    if (_posters.isEmpty && _bootTries < 3 && AppConfig.tmdbToken.isNotEmpty) {
      _bootTries++;
